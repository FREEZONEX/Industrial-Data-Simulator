...
## 🧩 项目简介
本项目是一个工业数据源模拟器，用于模拟数据中心的温度与散热流程，帮助任何对 UNS（Unified Namespace） 感兴趣的用户轻松部署并构建完整的 UNS 架构环境。数据源主要分为两类：  
### 1️⃣ 设备类数据源
| 设备名称                            | 协议               |
| ------------------------------     | ---------------- |
| 冷却塔（CT-301）                     | Modbus TCP       |
| 冷水机组（Chiller-201）              | Modbus TCP       |
| 泵（CDWP-301 / CHWP-201）           | OPC UA           |
| 计算机房空气处理器（CRAH-101）         | BACnet           |
| 服务器机柜（Rack-A01 / Rack-A02 / …）| MQTT             |
### 2️⃣ 软件系统数据源
| 系统名称      | 协议/访问方式            |
| -------     | --------------- |
