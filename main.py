...
bacnet_device = BacnetServer(crah_101, deviceId=1234, port=47808, update_interval=SERVER_UPDATE_INTERNAL)
modbus_server = ModbusServer(ct_301, chiller_201, address=("0.0.0.0", 5020), update_interval=SERVER_UPDATE_INTERNAL)
...
order_api = ServiceOrderAPI(db_config)

async def main():
    opcua_thread = threading.Thread(
        target=opcua_server.run, 
        args=(SERVER_UPDATE_INTERNAL,),
        daemon=True)
    opcua_thread.start()
    ...
    for dev in devices:
        random_update(dev, interval=RANDOM_UPDATE_INTERVAL)
...
def random_update(obj, interval=RANDOM_UPDATE_INTERVAL):
    def updater():
        while True:
            with obj._lock:
